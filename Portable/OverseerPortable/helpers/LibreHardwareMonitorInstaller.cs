...
namespace Overseer.Helpers
{
    public static class LibreHardwareMonitorInstaller
    {
        ...
        {
            // Prefer installer/setup variants for PawnIO first so the installer candidate is chosen
            "PawnIO_setup.exe",
            "pawnio_setup.exe",
            "pawnio.exe",
        ...
        private static bool TryInstallHelperInternal()
        {
            ...
            try
            {
                // Don't silently open the browser in unattended runs; ask the user first when possible.
                if (Application.Current != null)
                {
                    Application.Current.Dispatcher.Invoke(() =>
                    {
                        try
                        {
                            var result = MessageBox.Show("Required helper binaries were not found. Open LibreHardwareMonitor releases page to download them?", "Helper Missing", MessageBoxButton.YesNo, MessageBoxImage.Information);
                        ...
                        {
                            Debug.WriteLine($"Failed to prompt or open releases page: {ex}");
                        }
                    });
                }
                else
                {
                    // Non-UI environment: log intent but do not force the browser to open.
                    Debug.WriteLine($"Helper binaries missing; releases page: {ReleasesUrl}");
                }
            }
            catch (Exception ex)
            {
                Debug.WriteLine($"Failed to handle missing helper flow: {ex}");
            }
        ...
        private static (bool CanRun, string StatusText) VerifyInstallerChecksumIfPresent(string installerPath)
        {
            ...
            if (string.IsNullOrWhiteSpace(shaFile) || !File.Exists(shaFile))
            {
                ...
                else
                {
                    ...
                    if (any.Length > 0)
                    {
                        // Prefer files that contain the base name; do NOT pick an unrelated sha file.
                        var match = any.FirstOrDefault(f => Path.GetFileName(f).IndexOf(baseNoExt, StringComparison.OrdinalIgnoreCase) >= 0);
                        if (match != null) shaFile = match;
                    }
                    else
                    {
                        // Try parent helpers folder if present, but only accept files that match the base name
                        try
                        {
                            ...
                            if (!string.IsNullOrWhiteSpace(parent))
                            {
                                ...
                                if (anyParent.Length > 0)
                                {
                                    ...
                                    if (matchParent != null) shaFile = matchParent;
                                }
                            }
                        }
                        catch { }
            ...
            string expectedRaw = File.ReadAllText(shaFile).Trim();
                // Try to extract a 64-hex SHA256 token from the checksum file to be robust against different file formats
                string expected = string.Empty;
                try
                {
                    var m = Regex.Match(expectedRaw, "[A-Fa-f0-9]{64}");
                    if (m.Success)
                    {
                        expected = m.Value;
                        Debug.WriteLine($"Extracted SHA token from checksum file: {expected}");
                    }
                    else
                    {
                        // Fallback: take the first whitespace-separated token (legacy behavior)
                        if (expectedRaw.Contains(' ')) expected = expectedRaw.Split(new[] { ' ', '\t', '\r', '\n' }, StringSplitOptions.RemoveEmptyEntries)[0];
                        else expected = expectedRaw.Trim();
                    }
                }
                catch (Exception ex)
                {
                    Debug.WriteLine($"Failed to parse checksum file '{shaFile}': {ex}");
                    expected = expectedRaw.Trim();
                }

                if (string.IsNullOrWhiteSpace(expected)) return (false, "Checksum file empty");
                ...
                using (var stream = File.OpenRead(installerPath))
                {
                    ...
                    string expectedNorm = expected.Replace(" ", string.Empty).ToLowerInvariant();
                    Debug.WriteLine($"Checksum verification details: installer='{installerPath}', shaFile='{shaFile}', expected='{expectedNorm}', actual='{actual}'");
                    if (actual.Equals(expectedNorm))
                    {
                    ...
                    {
                        string msg = $"SHA256 mismatch. Expected {expectedNorm}, got {actual} (installer='{installerPath}', shaFile='{shaFile}')";
                        Debug.WriteLine(msg);
                        return (false, msg);
                    }
                }
